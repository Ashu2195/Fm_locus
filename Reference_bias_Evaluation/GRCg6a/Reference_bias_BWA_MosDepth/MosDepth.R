read.table(file="Dup1_hap1.fastq.sorted_cov.regions.bed",header=F)->D1H1
read.table(file="Dup1_hap2.fastq.sorted_cov.regions.bed",header=F)->D1H2
read.table(file="Dup2_hap1.fastq.sorted_cov.regions.bed",header=F)->D2H1
read.table(file="Dup2_hap2.fastq.sorted_cov.regions.bed",header=F)->D2H2
read.table(file="other_RG_cov.regions.bed",header=F)->OR
read.table(file="allreads_cov.regions.bed",header=F)->AR
read.table(file="IlluminaReads_cov.regions.bed",header=F)->IR

tiff("MosDepth_Read_Coverage.tiff",width = 2980, height = 3080, res = 170)
par(mfrow=c(7,1))
par(mar=c(5,5,4,1)) 
